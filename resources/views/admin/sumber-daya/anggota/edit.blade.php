                ...
                <form id="anggota-edit" action="{{ route('anggota.update', $anggota->id) }}" method="post"
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" value="{{ $anggota->nama }}" class="form-control @error('nama')
                                    is-invalid
                                @enderror" name="nama" id="nama" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="nip">NIP/NIDN <sup class="text-danger"></sup></label>
                            <input type="number" value="{{ $anggota->nip }}" class="form-control @error('nip')
                                                            is-invalid
                                                        @enderror" name="nip" id="nip">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="email">Alamat Email <sup class="text-danger">*</sup></label>
                            <input type="email" value="{{ $anggota->email }}" class="form-control @error('email')
                                                                is-invalid
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="jenis_kelamin" class="form-control" id="">
                                <option value="lk" @if ($anggota->jenis_kelamin == 'lk')
                                    selected
                                @endif>Laki-laki</option>
                                <option @if ($anggota->jenis_kelamin == 'pr')
                                    selected
                                @endif value="pr">Perempuan</option>
                            </select>
                        </div>
                    </div>
